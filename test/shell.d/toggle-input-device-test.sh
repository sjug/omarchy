...
run_migration() {
  HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" HYPRCTL_LOG="$log_file" OMARCHY_PATH="$ROOT" \
    PATH="$stub_dir:$ROOT/bin:$PATH" \
...
# The overlay owns these same toggle files but must not run product migrations.
# Exercise its entry point too, keeping all reloads behind the hyprctl stub.
printf 'hl.device({ name = "overlay-touchpad", enabled = false })\n' >"$state_lua"
: >"$log_file"
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" HYPRCTL_LOG="$log_file" OMARCHY_PATH="$ROOT" \
  PATH="$stub_dir:$ROOT/bin:$PATH" \
  bash -euo pipefail "$ROOT/migrations/desktop-overlay/1787618700.sh" >/dev/null
[[ $(<"$name_file") == "overlay-touchpad" && ! -e $state_lua ]] ||
  fail "overlay migration preserves a legacy disabled device as data"
grep -qx 'reload' "$log_file" || fail "overlay migration reapplies the recovered disable"
: >"$log_file"
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" HYPRCTL_LOG="$log_file" OMARCHY_PATH="$ROOT" \
  PATH="$stub_dir:$ROOT/bin:$PATH" \
  bash -euo pipefail "$ROOT/migrations/desktop-overlay/1787618700.sh" >/dev/null
[[ ! -s $log_file ]] || fail "overlay migration reloads again with nothing to migrate"
pass "overlay input-device migration shares the idempotent product repair without its migration stream"
